...
def load_drum_srcs(idx=1):
    ...
    for drum_note_name in DRUM_NAMES:
        ...
        src, _ = librosa.load(os.path.join(DATA_PATH, filename), sr=SR_WAV, duration=dura) #src.shape=(SR_WAV,)
        srcs.append(src)

    srcs = np.array(srcs) #srcs.shape=(len(DRUM_NAMES),SR_WAV)
    # inst_srcs = torch.from_numpy(srcs)
    inst_srcs = tf.constant(srcs, dtype=np.float32, name="drum_srcs")
    return inst_srcs
